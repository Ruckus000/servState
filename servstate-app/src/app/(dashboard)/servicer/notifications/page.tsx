...
const notificationIcons: Record<NotificationType, React.ReactNode> = {
  ...
  task: <ClipboardList className="h-5 w-5 text-primary" />,
...
export default function ServicerNotificationsPage() {
  const [notifications, setNotifications] = useState(() =>
    getNotificationsByRole('servicer')
  );
  ...
  const handleMarkAsRead = (id: string) => {
    markAsRead(id);
    setNotifications(getNotificationsByRole('servicer'));
  };

  const handleMarkAllAsRead = () => {
    markAllAsRead('servicer');
    setNotifications(getNotificationsByRole('servicer'));
  };
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            {notification.loan_id && (
              <Badge variant="outline" className="text-xs">
                Loan #{notification.loan_id.replace('loan_', '1000567')}
              </Badge>
            )}
          </div>
        </div>
